...
* 绝对定位具有以下属性(position: absolute)：(TRBL-top/right/bottom/left)
	...
	- 如果没有TRBL: 元素__脱离正常文档流__，并且不影响正常文档流，就像在空中飘一样，但是位置依旧在元素position:static时的位置。
	- 如果设定TRBL
		+ 父级没有设定非static的position属性，以浏览器左上角为原始点进行定位，位置将由TRBL决定。
		+ 父级设定非static的position属性，则以父级的左上角为原点进行定位，位置由TRBL决定。
	...
	1. 设定TRBL
	2. 父级设定非static的Position属性
 
* 绝对定位(position:fixed):

	- 如果没有TRBL: 元素__脱离正常文档流__，并且不影响正常文档流，就像在空中飘一样，但是位置依旧在元素position:static时的位置。

	- 如果设定TRBL: 元素以浏览器窗口左上角为原点进行定位。
	
* 相对定位(position: relative)： 是相对于其正常流的位置为原点进行定位。
